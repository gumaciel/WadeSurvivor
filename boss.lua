...
local spritezombie  = "data/sprites/boss/"

ClassBoss = {}

function ClassBoss.new(_x, _y)
	return {
		zombie = {
			direction = {
				x = nil,
			...
			anim = false,
			sound = {
				[1] = la.newSource("data/sounds/zombie-20.wav"),
				[2] = la.newSource("data/sounds/zombie-2.wav")
			},
			hp = {
				points = 100,
				[1] = {
			...
			animation = {
				up = nil, down = nil, left = nil, right = nil, atkleft = nil, atkright = nil
			},
			sprite = {
				...
				atk = lg.newImage(spritezombie .. "atk.png")
			}
		},
		...
		addZombie = function (self)
			...
			zombie.w = (zombie.sprite.down:getWidth()/5)
			zombie.h = (zombie.sprite.down:getHeight())

			local animZombie = anim.newGrid(
				zombie.sprite.down:getWidth()/5,
				zombie.sprite.down:getHeight(),
				zombie.sprite.down:getWidth(),
				zombie.sprite.down:getHeight()
			)
			zombie.animation.up = anim.newAnimation(animZombie ("1-5", 1), 0.1)
			zombie.animation.down = anim.newAnimation(animZombie ("1-5", 1), 0.1)
			local animZombieleft_right = anim.newGrid(
				zombie.sprite.left_right:getWidth()/6,
				zombie.sprite.left_right:getHeight(),
				zombie.sprite.left_right:getWidth(),
				zombie.sprite.left_right:getHeight()
			)
			zombie.animation.left = anim.newAnimation(animZombieleft_right ("1-6", 1), 0.1)
			zombie.animation.right = zombie.animation.left:clone():flipH()
			local animZombieatk = anim.newGrid(
				zombie.sprite.atk:getWidth()/7,
				zombie.sprite.atk:getHeight(),
				zombie.sprite.atk:getWidth(),
				zombie.sprite.atk:getHeight()
			)
			zombie.animation.atkright = anim.newAnimation(animZombieatk ("1-7", 1), 0.1)
			zombie.animation.atkleft = zombie.animation.atkright:clone():flipH()
		...
		mov_zombie = function(self, dt)
			...
			if checkCollision(
				...
				zombie.animation.atkleft:update(dt) 
				zombie.animation.atkright:update(dt)
		...
		draw = function(self)
			...
			local IMGx, IMGy 			= zx - zw/1.35, zy - zh*3
												 ...
												 zombie.hp[1].x, zombie.hp[1].y = zombie.x - 7, zombie.y - 100
			love.graphics.draw(zombie.hp[1].img, zombie.hp[1].x, zombie.hp[1].y, 0, 1, 1)

												 zombie.hp[2].x, zombie.hp[2].y = zombie.x - 6 , zombie.y - 99 
			love.graphics.draw(zombie.hp[2].img, zombie.hp[2].x, zombie.hp[2].y, 0, zombie.hp.points/100, 1)


			local directionX, directionY = zombie.direction.x, zombie.direction.y
			if zombie.live == true then
				if 		not directionY and 	not directionX and not zombie.attacking then
					zombie.animation.down:draw (zombie.sprite.down, IMGx, IMGy, 0, 1, 1)
				elseif 	not directionX and 		directionY and not zombie.attacking then
					zombie.animation.left:draw (zombie.sprite.left_right, IMGx, IMGy, 0, 1, 1)
				elseif 		directionX and 	not directionY and not zombie.attacking then
					zombie.animation.right:draw (zombie.sprite.left_right, IMGx, IMGy, 0, 1, 1)
				elseif 		directionY and 		directionX and not zombie.attacking then
					zombie.animation.up:draw (zombie.sprite.up, IMGx, IMGy, 0, 1, 1)				
				elseif directionX and zombie.attacking then
					zombie.animation.atkright:draw (zombie.sprite.atk, IMGx, IMGy, 0, 1, 1)				
				elseif not directionX and zombie.attacking then
					zombie.animation.atkleft:draw (zombie.sprite.atk, IMGx, IMGy, 0, 1, 1)				
				end
			end
		end
	}
